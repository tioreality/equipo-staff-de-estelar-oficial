...
panel/auth.py
-------------
Login con Discord (OAuth2) para el panel del equipo Estelar.
...
4. Con el ID de esa persona, usamos el TOKEN DEL BOT (no el suyo) para
   ...
   en el servidor de Estelar?" -> GET /guilds/{guild_id}/members/{user_id}.
...
Mismo patrón que panel/auth.py de TAMAGO, sin el bloque de debug
temporal que ese archivo tenía marcado para borrar.
"""
...
async def user_is_admin(config: PanelConfig, user_id: str) -> bool:
    ...
    servidor del equipo Estelar (GUILD_ID). El usuario nunca ve ni
    maneja el token del bot: esta consulta pasa solo por el backend.
    """
    async with httpx.AsyncClient(timeout=10) as client:
