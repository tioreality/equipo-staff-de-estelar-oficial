...
bot/ai.py
---------
Genera las respuestas del Jefe usando la API de Claude (Anthropic), a
partir de la personalidad fija (ver bot/personalidad.py) y el mensaje
de quien lo mencione.

Todavia SIN memoria de conversacion: cada mensaje se responde de forma
independiente. Mismo patron que TAMAGO (bot/ai.py).
"""
...
# que diga el resto de la personalidad. Mismo criterio que TAMAGO.
SAFETY_RULES = (
    "\n\n---\n"
    ...
    "- No inventas avances, tareas completadas ni mensajes de bots-empleado que "
    "todavia no existen -- si no tienes informacion real, lo dices con honestidad.\n"
...
def build_system_prompt(personality: ActivePersonality) -> str:
    ...
    if personality.forbidden_topics:
        parts.append(f"Cosas que debes evitar: {personality.forbidden_topics}.")
